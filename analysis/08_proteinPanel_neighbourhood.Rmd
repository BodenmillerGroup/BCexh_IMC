...
sce_protein <- readRDS("output/ProteinPanel/sce_ProteinPanel_subtypes.rds")
...
#preTLS_nr <- c(17,21,54,60,68)
nonTLS_nr <- c(1:77)
nonTLS_nr <- setdiff(nonTLS_nr, TLS_nr)
#nonTLS_nr <- setdiff(nonTLS_nr, preTLS_nr)
```
...
```{r pairwise-classic-all, fig.width=10, fig.height=10}
# Prepare the data
...
```{r pairwise-relative-all, fig.width=10, fig.height=10}
...
```{r pairwise-classic-TLS, fig.width=10, fig.height=10}
# Prepare the data
pmat = dcast(dat_p, 'FirstLabel ~ SecondLabel', value.var = 'sigval', fun.aggregate = sum,
             fill=0, drop=F)
...
```{r pairwise-relative-TLS, fig.width=10, fig.height=10}
# Prepare data table with number of images containing both interaction partners
dat_p$present <- 1
nmat = dcast(dat_p, 'FirstLabel ~ SecondLabel', value.var = 'present', fun.aggregate = sum,
             fill=0, drop=F)
...
```{r pairwise-classic-nonTLS, fig.width=10, fig.height=10}
# Prepare the data
pmat = dcast(dat_p, 'FirstLabel ~ SecondLabel', value.var = 'sigval', fun.aggregate = sum,
             fill=0, drop=F)
...
```{r pairwise-relative-nonTLS, fig.width=10, fig.height=10}
# Prepare data table with number of images containing both interaction partners
dat_p$present <- 1
nmat = dcast(dat_p, 'FirstLabel ~ SecondLabel', value.var = 'present', fun.aggregate = sum,
             fill=0, drop=F)
