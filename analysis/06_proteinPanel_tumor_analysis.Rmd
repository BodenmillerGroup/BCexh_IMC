...
```{r tumor-clusters-expression-heatmap, fig.width=5, fig.height=8}
...
```{r TIG-marker-comparison-patientmeans, fig.width=5, fig.height=6}
...
```{r HLA_ABC-HER2-correlation, fig.width=3.5, fig.height=3}
...
cluster.averages[!(rownames(cluster.averages) %in% not.measured),]
...
# Define cell status based on Ki67, Carbonic Anhydrase IX and Cleaved Caspase/Cleaved PARP expression
...
```{r tumor-cell-status-TIG, fig.width=5, fig.height=2.5}
...
```{r tumor-cell-status-barplot, fig.width=4, fig.height=3}
...
# Add tumor cell status to main SCE object as "celltype"

```{r update-main-SCE}
sce_protein <- readRDS(file = "output/ProteinPanel/sce_ProteinPanel_celltypes.rds")

tumor_ids <- colnames(sce_tumor)
sce_protein[,tumor_ids]$celltype <- sce_tumor$cell_status
table(sce_protein$celltype)
