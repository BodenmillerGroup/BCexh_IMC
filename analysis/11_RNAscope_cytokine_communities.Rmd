...
## Single vs Cluster: Proportion of cytokine-expressing cells

Exclude TLS and pre-TLS images for TIG comparison.
...
```{r community-celltypes-nonexpressing}
...
```{r community-enrichment-nonexpressing}
...
for (i in cytokine_channels) {
  comm_name_i <- tolower(paste0(i,"only_comm"))
  cur_sce_i <- cur_sce[which(cur_sce[,comm_name_i] != 0 & cur_sce[,str_detect(colnames(cur_sce), regex(paste0(i,"_expression"), ignore_case = TRUE))] == 0),]
...
for (k in (colnames(coldata_RNA)[57:67])) {
