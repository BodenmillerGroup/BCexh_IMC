    ...
    for(j in unique(cur_sce_i$ImageNumber)) {
      ...
      if (ncol(sce_sub_ji) > 0) {
        ...
        for(k in unique(cur_sce_i$celltype)) {
          count_kimg <- table(cur_sce$celltype, cur_sce$ImageNumber)[k, j]  # Number of this cell in the image
...
Heatmap(enrich_corr, column_title = "% of images with significant enrichment/depletion", row_title = "Cytokine milieu", row_title_side = "right", heatmap_legend_param = list(title = ""))
...
# Heatmap(enrich_corr_celltype, na_col = "gray")
