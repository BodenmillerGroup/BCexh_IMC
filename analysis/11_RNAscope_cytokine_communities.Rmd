...
for (i in cytokine_channels) {
  cur_sce_i <- coldata_myeloid[which(coldata_myeloid[,str_detect(colnames(coldata_myeloid), regex(paste0(i,"only_comm"), ignore_case = TRUE))] != 0 & coldata_myeloid[,str_detect(colnames(coldata_myeloid), regex(paste0(i,"only_clust"), ignore_case = TRUE))] == 0),]
...
ggplot(perc_total, aes(environment, percent_cytokine_expressing, fill = environment))+
  ...
  ggtitle("Cytokine-expressing myeloid percentage in the respective communities")
...
## Compare cell type proportions in cytokine communities: RNA vs protein panel
...
for (k in (colnames(coldata_RNA)[55:64])) {
...
enrichment_sample <- data.frame(matrix(ncol=4, nrow = 0))

for (k in colnames(coldata_protein)[24:33]) {
  for(j in unique(as.character(coldata_protein$sample))) {
    sce_sub_k <- coldata_protein[coldata_protein[,k] == 1,]
    sce_sub_kj <- sce_sub_k[sce_sub_k[,"sample"] == j,]
    prop_kj <- prop.table(table(sce_sub_kj$subtype))
    prop_j <- prop.table(table(sce_protein[,which(sce_protein$sample == j)]$subtype))[names(prop_kj)]
    enrichment_j <-  prop_kj/prop_j 
    if(length(prop_kj > 0)) {
          j_df <- data.frame(sample = j,
                      cytokine = k,
                      enrichment = enrichment_j)
    enrichment_sample <- rbind(enrichment_sample, j_df)
    }
  }
}

colnames(enrichment_sample)[3:4] = c("subtype", "enrichment")

ggplot(enrichment_sample, aes(cytokine, enrichment, fill = cytokine))+
  geom_boxplot()+
...
#Calculate enrichment of T cell subtypes in environment k
...
ggplot(enrichment_TNK, aes(cytokine, enrichment, fill = cytokine))+
  geom_boxplot()+
  facet_wrap(~T_subtype, scales = "free", ncol = 4)+
  theme(panel.background = element_blank(),
        axis.text.x = element_blank(),
        panel.border = element_rect(color = "black", fill = NA, size = 1),
        strip.background = element_blank(),
